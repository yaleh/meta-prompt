...
def evaluate_system_message(config, system_message, user_message, executor_model_name, executor_temperature):
    ...
    executor model based on the provided user message.
    ...
    Args:
        ...
        executor_temperature (float): The temperature setting for the executor model.

    Returns:
        str: The content of the output generated by the LLM based on the system
            message and user message, or an empty string if an error occurs.
...
def compare_outputs(expected_output, actual_output):
    ...
    return [
        ...
        for token in d.compare(expected_output.split(), actual_output.split())
    ]